...
Web 入口已额外加入三个英语学习交互模式：

| 模块 | 借鉴方向 | 已落地能力 |
|------|----------|------------|
| 阅读透镜 | 类 TransLens 的阅读选词/选句即时解析 | 粘贴英文材料、选中文本解析、提取疑似生词、一键收藏到生词本、浏览器朗读 |
| 单词老师 | 类 word-teacher 的词汇教学卡片 | 单词释义、场景例句、易错提醒、小测题、朗读、保存到 `user_word` |
| Echo Loop | 类 Echo-Loop 的跟读循环训练 | 标准音循环播放、浏览器录音、上传 MinIO、关联口语 Agent 评分与历史记录 |
