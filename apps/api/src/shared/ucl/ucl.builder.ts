 ...
 * 供所有 AI Engine 和路由复用。
 */
import type { UserCapabilityLevel, WeakAreas, ErrorPattern } from '@englishi/shared-types';

/**
 * 将 DB 能力模型行转换为 UCL 对象
 */
export function buildUCL(userId: string, model: {
  ...
  return {
    ...
    masteredGrammar: (model.masteredGrammar as string[] | null) ?? [],
    notYetGrammar: [],  // 从语法知识图谱动态计算，此处默认为空
    weakAreas: (model.weakAreas as WeakAreas | null) ?? {
      grammar: [], listening: [], writing: [], speaking: [], vocabulary: [],
    },
    errorPatterns: (model.errorPatterns as ErrorPattern[] | null) ?? [],
 ...
 */
export function extractDifficultyParams(ucl: UserCapabilityLevel, skill: 'reading' | 'listening' | 'writing' | 'speaking') {
 ...
 */
function estimateVocabSizeFromCefr(cefr: number): number {
  const mapping: Record<number, number> = { 1: 500, 2: 1200, 3: 2500, 4: 4500, 5: 7000, 6: 10000 };
